...
rl.on('line', async (line) => {
    ...
    switch (cmd) {
        ...
        case 'run_so':
            if (!args[0]) {
                console.log(`${C.red}Usage: so <path_to_so_file_on_pc> [entrypoint]${C.reset}`);
            } else {
                await handleRunSo(args[0], args[1] || '');
            }
            break;
...
async function handleRunSo(filePath, entrypoint) {
    ...
    if (!fs.existsSync(resolvedPath)) {
        console.log(`${C.red}❌ File not found on PC: ${resolvedPath}${C.reset}`);
        return;
    }
    ...
    console.log(`\n${C.cyan}📦 Uploading & executing '${filename}' (${sizeKb} KB) on iPhone...${C.reset}`);
    ...
    if (res && res.log) {
        ...
        const savedFile = res.file || 'test_so.log';
        const targetPath = path.join(LOGS_DIR, savedFile);
        fs.writeFileSync(targetPath, res.log, 'utf8');
        console.log(`\n${C.green}💾 Execution log saved to: ${C.bold}${targetPath}${C.reset}\n`);
    } else {
