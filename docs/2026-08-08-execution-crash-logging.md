...
## ⚠️ BUG CẦN SỬA (chưa fix — CI run `31247865195` fail)
`pthread_jit_write_protect_np` **không có trên iOS** (chỉ có trên macOS):
```
src/elf_loader.cpp:242: error: 'pthread_jit_write_protect_np' is unavailable: not available on iOS
src/elf_loader.cpp:258: error: 'pthread_jit_write_protect_np' is unavailable: not available on iOS
```
- **Nguyên nhân**: commit `fbb1106` thêm `MAP_JIT` + `pthread_jit_write_protect_np()`
  bọc trong `#if defined(__APPLE__)`, nhưng API này bị Apple mark unavailable
  cho iOS (chỉ dùng được trên macOS). iOS device dùng cơ chế khác.
- **Hướng fix (khi được yêu cầu)**:
  - Trên iOS, LiveContainer bật JIT qua debugger → **page RWX thường chạy được**,
    KHÔNG cần `MAP_JIT`/`pthread_jit_write_protect_np`.
  - Nên phân biệt `TARGET_OS_OSX` vs `TARGET_OS_IOS`:
    - macOS: `MAP_JIT` + `pthread_jit_write_protect_np` toggle.
    - iOS: `mmap` RWX thường (như code trước commit `fbb1106`), chỉ giữ
      `sys_icache_invalidate`.
  - Dùng `#include <TargetConditionals.h>` và guard `#if TARGET_OS_OSX` cho phần
    `pthread_jit_write_protect_np`.
- File cần sửa: `src/elf_loader.cpp` (khối `map()` quanh dòng 220–260).
