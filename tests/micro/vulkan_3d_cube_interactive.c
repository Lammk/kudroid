...
#define LOG_TAG "Vulkan3DCube"
#define LOGI(...) __android_log_print(3, LOG_TAG, __VA_ARGS__)
#define LOGE(...) __android_log_print(6, LOG_TAG, __VA_ARGS__)

#include "cube_gl_model_vert_spv.h"
#include "cube_gl_model_frag_spv.h"
...
typedef struct VkMemoryAllocateInfo {
    uint32_t sType; const void* pNext; VkDeviceSize allocationSize; uint32_t memoryTypeIndex;
} VkMemoryAllocateInfo;

typedef struct VkMappedMemoryRange {
    uint32_t sType; const void* pNext; VkDeviceMemory memory; VkDeviceSize offset; VkDeviceSize size;
} VkMappedMemoryRange;

typedef struct VkMemoryType {
    VkFlags propertyFlags; uint32_t heapIndex;
} VkMemoryType;
...
typedef struct VkVertexInputBindingDescription {
    uint32_t binding; uint32_t stride; uint32_t inputRate;
...
typedef struct VkPipelineDynamicStateCreateInfo {
    uint32_t sType; const void* pNext; VkFlags flags;
    uint32_t dynamicStateCount; const uint32_t* pDynamicStates;
...
// 24 Đỉnh của 6 mặt Khối Lập Phương 3D (Khớp 100% OpenGL Cube Model)
...
int kudroid_test_main(void) {
    LOGI("=================================================");
    LOGI("🌋 [HARDCORE 3D VULKAN INDEXED CUBE (OPENGL ARCH)]");
    LOGI("=================================================");
    ...
    VkApplicationInfo appInfo = { VK_STRUCTURE_TYPE_APPLICATION_INFO, NULL, "Vulkan3DCube", 1, "KuDroidEngine", 1, 0x00401000 };
    VkInstanceCreateInfo instInfo = { VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO, NULL, 0, &appInfo, 0, NULL, 2, instanceExts };

    VkInstance instance = NULL;
    fnCreateInstance(&instInfo, NULL, &instance);
    ...
    // ========================================================
    // 🌟 VBO (24 Đỉnh) & IBO (36 Indices) NHƯ OPENGL MODEL
    // ========================================================
    ...
    PFN_vkCreateBuffer fnCreateBuffer = (PFN_vkCreateBuffer)(fnGetDeviceProcAddr ? fnGetDeviceProcAddr(device, "vkCreateBuffer") : vkGetInstanceProcAddr(instance, "vkCreateBuffer"));
    PFN_vkGetBufferMemoryRequirements fnGetBufferMemReq = (PFN_vkGetBufferMemoryRequirements)(fnGetDeviceProcAddr ? fnGetDeviceProcAddr(device, "vkGetBufferMemoryRequirements") : vkGetInstanceProcAddr(instance, "vkGetBufferMemoryRequirements"));
    ...
    PFN_vkAllocateMemory fnAllocMem = (PFN_vkAllocateMemory)(fnGetDeviceProcAddr ? fnGetDeviceProcAddr(device, "vkAllocateMemory") : vkGetInstanceProcAddr(instance, "vkAllocateMemory"));
    PFN_vkBindBufferMemory fnBindBufferMem = (PFN_vkBindBufferMemory)(fnGetDeviceProcAddr ? fnGetDeviceProcAddr(device, "vkBindBufferMemory") : vkGetInstanceProcAddr(instance, "vkBindBufferMemory"));
    PFN_vkMapMemory fnMapMem = (PFN_vkMapMemory)(fnGetDeviceProcAddr ? fnGetDeviceProcAddr(device, "vkMapMemory") : vkGetInstanceProcAddr(instance, "vkMapMemory"));
    PFN_vkUnmapMemory fnUnmapMem = (PFN_vkUnmapMemory)(fnGetDeviceProcAddr ? fnGetDeviceProcAddr(device, "vkUnmapMemory") : vkGetInstanceProcAddr(instance, "vkUnmapMemory"));
    PFN_vkFlushMappedMemoryRanges fnFlushMem = (PFN_vkFlushMappedMemoryRanges)(fnGetDeviceProcAddr ? fnGetDeviceProcAddr(device, "vkFlushMappedMemoryRanges") : vkGetInstanceProcAddr(instance, "vkFlushMappedMemoryRanges"));

    VkPhysicalDeviceMemoryProperties memProps;
    fnGetMemProps(gpu, &memProps);

    // 1. Tạo VBO
    VkDeviceSize vboSize = sizeof(kCubeVertices);
    VkBufferCreateInfo vboInfo = { VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO, NULL, 0, vboSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_SHARING_MODE_EXCLUSIVE, 0, NULL };
    VkBuffer vertexBuffer = NULL;
    fnCreateBuffer(device, &vboInfo, NULL, &vertexBuffer);
    VkMemoryRequirements vboReq;
    fnGetBufferMemReq(device, vertexBuffer, &vboReq);

    uint32_t memTypeIndex = 0;
    for (uint32_t i = 0; i < memProps.memoryTypeCount; ++i) {
        if ((vboReq.memoryTypeBits & (1 << i)) && (memProps.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)) {
            memTypeIndex = i;
            break;
        }
    }

    VkMemoryAllocateInfo vboAlloc = { VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO, NULL, vboReq.size, memTypeIndex };
    VkDeviceMemory vertexBufferMemory = NULL;
    fnAllocMem(device, &vboAlloc, NULL, &vertexBufferMemory);
    fnBindBufferMem(device, vertexBuffer, vertexBufferMemory, 0);

    void* vboPtr = NULL;
    fnMapMem(device, vertexBufferMemory, 0, vboSize, 0, &vboPtr);
    if (vboPtr) {
        memcpy(vboPtr, kCubeVertices, (size_t)vboSize);
        if (fnFlushMem) {
            VkMappedMemoryRange r = { VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE, NULL, vertexBufferMemory, 0, vboSize };
            fnFlushMem(device, 1, &r);
        }
        fnUnmapMem(device, vertexBufferMemory);
    }

    // 2. Tạo IBO (Index Buffer)
    VkDeviceSize iboSize = sizeof(kCubeIndices);
    VkBufferCreateInfo iboInfo = { VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO, NULL, 0, iboSize, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_SHARING_MODE_EXCLUSIVE, 0, NULL };
    VkBuffer indexBuffer = NULL;
    fnCreateBuffer(device, &iboInfo, NULL, &indexBuffer);
    VkMemoryRequirements iboReq;
    fnGetBufferMemReq(device, indexBuffer, &iboReq);

    VkMemoryAllocateInfo iboAlloc = { VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO, NULL, iboReq.size, memTypeIndex };
    VkDeviceMemory indexBufferMemory = NULL;
    fnAllocMem(device, &iboAlloc, NULL, &indexBufferMemory);
    fnBindBufferMem(device, indexBuffer, indexBufferMemory, 0);

    void* iboPtr = NULL;
    fnMapMem(device, indexBufferMemory, 0, iboSize, 0, &iboPtr);
    if (iboPtr) {
        memcpy(iboPtr, kCubeIndices, (size_t)iboSize);
        if (fnFlushMem) {
            VkMappedMemoryRange r = { VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE, NULL, indexBufferMemory, 0, iboSize };
            fnFlushMem(device, 1, &r);
        }
        fnUnmapMem(device, indexBufferMemory);
    }

    LOGI("✔ VBO (%lu B) & IBO (%lu B) Ready & Flushed to Metal VRAM", (unsigned long)vboSize, (unsigned long)iboSize);

    extern void* objc_autoreleasePoolPush(void);
    extern void objc_autoreleasePoolPop(void*);

    void* initPool = objc_autoreleasePoolPush();
    ...
    VkShaderModuleCreateInfo vsInfo = { VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO, NULL, 0, kCubeGlModelVertSpirvSize, kCubeGlModelVertSpirv };
    VkShaderModule vertShader = NULL;
    VkResult resVS = fnCreateShaderModule(device, &vsInfo, NULL, &vertShader);

    VkShaderModuleCreateInfo fsInfo = { VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO, NULL, 0, kCubeGlModelFragSpirvSize, kCubeGlModelFragSpirv };
    VkShaderModule fragShader = NULL;
    VkResult resFS = fnCreateShaderModule(device, &fsInfo, NULL, &fragShader);

    // Pipeline Layout (Không Push Constants)
    ...
    VkResult resPL = fnCreatePipelineLayout(device, &plInfo, NULL, &pipelineLayout);

    // Vertex Input State
    VkVertexInputBindingDescription bindingDesc = { 0, sizeof(Vertex3D), VK_VERTEX_INPUT_RATE_VERTEX };
    VkVertexInputAttributeDescription attribDescs[2] = {
        { 0, 0, VK_FORMAT_R32G32B32_SFLOAT, 0 },                   // aPosition (offset 0)
        { 1, 0, VK_FORMAT_R32G32B32_SFLOAT, sizeof(float) * 3 }    // aColor (offset 12)
    };
    VkPipelineVertexInputStateCreateInfo vertexInputInfo = {
        VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO, NULL, 0,
        1, &bindingDesc, 2, attribDescs
    };
    ...
    uint32_t dynamicStates[2] = { 0, 1 };
    VkPipelineDynamicStateCreateInfo dynamicStateInfo = { VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO, NULL, 0, 2, dynamicStates };
    ...
    VkGraphicsPipelineCreateInfo pipelineInfo = {
        ...
        &colorBlending, &dynamicStateInfo, pipelineLayout, renderPass, 0, NULL, -1
    };
    ...
    VkResult resPipe = fnCreateGraphicsPipelines(device, NULL, 1, &pipelineInfo, NULL, &graphicsPipeline);

    LOGI("✔ Pipeline Audit: vs=%d (%p), fs=%d (%p), layout=%d (%p), pipe=%d (%p)",
         (int)resVS, vertShader, (int)resFS, fragShader, (int)resPL, pipelineLayout, (int)resPipe, graphicsPipeline);

    objc_autoreleasePoolPop(initPool);
    ...
    typedef void (*PFN_vkCmdBindIndexBuffer)(VkCommandBuffer, VkBuffer, VkDeviceSize, uint32_t);
    ...
    PFN_vkCmdSetViewport fnCmdSetViewport = (PFN_vkCmdSetViewport)(fnGetDeviceProcAddr ? fnGetDeviceProcAddr(device, "vkCmdSetViewport") : vkGetInstanceProcAddr(instance, "vkCmdSetViewport"));
    PFN_vkCmdSetScissor fnCmdSetScissor = (PFN_vkCmdSetScissor)(fnGetDeviceProcAddr ? fnGetDeviceProcAddr(device, "vkCmdSetScissor") : vkGetInstanceProcAddr(instance, "vkCmdSetScissor"));
    PFN_vkCmdBindVertexBuffers fnCmdBindVertexBuffers = (PFN_vkCmdBindVertexBuffers)(fnGetDeviceProcAddr ? fnGetDeviceProcAddr(device, "vkCmdBindVertexBuffers") : vkGetInstanceProcAddr(instance, "vkCmdBindVertexBuffers"));
    PFN_vkCmdBindIndexBuffer fnCmdBindIndexBuffer = (PFN_vkCmdBindIndexBuffer)(fnGetDeviceProcAddr ? fnGetDeviceProcAddr(device, "vkCmdBindIndexBuffer") : vkGetInstanceProcAddr(instance, "vkCmdBindIndexBuffer"));
    PFN_vkCmdPushConstants fnCmdPushConstants = (PFN_vkCmdPushConstants)(fnGetDeviceProcAddr ? fnGetDeviceProcAddr(device, "vkCmdPushConstants") : vkGetInstanceProcAddr(instance, "vkCmdPushConstants"));
    PFN_vkCmdDrawIndexed fnCmdDrawIndexed = (PFN_vkCmdDrawIndexed)(fnGetDeviceProcAddr ? fnGetDeviceProcAddr(device, "vkCmdDrawIndexed") : vkGetInstanceProcAddr(instance, "vkCmdDrawIndexed"));
    ...
    LOGI("=================================================");
    LOGI("🌟 3D VULKAN INDEXED CUBE IS LIVE (OPENGL ARCH)");
    ...
    float aspect = (float)swapExtent.width / (float)(swapExtent.height > 0 ? swapExtent.height : 1);
    ...
    for (int frame = 0; frame < 1500; ++frame) {
        ...
        fnAcquireNextImage(device, swapchain, 1000000000ULL, NULL, NULL, &imageIndex);
        ...
        if (fnCmdSetViewport) fnCmdSetViewport(cmd, 0, 1, &viewport);
        if (fnCmdSetScissor) fnCmdSetScissor(cmd, 0, 1, &scissor);

        // Bind VBO & IBO
        VkDeviceSize offsets[] = { 0 };
        fnCmdBindVertexBuffers(cmd, 0, 1, &vertexBuffer, offsets);
        fnCmdBindIndexBuffer(cmd, indexBuffer, 0, VK_INDEX_TYPE_UINT16);

        // Draw Indexed 36 indices (Khớp 100% lệnh glDrawElements của OpenGL)
        fnCmdDrawIndexed(cmd, 36, 1, 0, 0, 0);
    ...
    LOGI("=================================================");
    LOGI("🎉 3D VULKAN INDEXED CUBE PASSED 100%!");
