...
int kudroid_test_main(void) {
    LOGI("=================================================");
    LOGI("🌟 [HARDCORE MINIMAL VULKAN 1.1 RGB TRIANGLE]");
    LOGI("=================================================");
    ...
    VkApplicationInfo appInfo = { VK_STRUCTURE_TYPE_APPLICATION_INFO, NULL, "VulkanTri", 1, "KuDroidEngine", 1, 0x00401000 };
    ...
    if (swapExtent.width == 0 || swapExtent.width == 0xFFFFFFFF) {
        swapExtent.width = 414;
        swapExtent.height = 896;
    }
    ...
    // Render Pass with Subpass Dependency
    ...
    fnCreateShaderModule(device, &vsInfo, NULL, &vertShader);
    ...
    fnCreateShaderModule(device, &fsInfo, NULL, &fragShader);
    ...
    fnCreatePipelineLayout(device, &plInfo, NULL, &pipelineLayout);
    ...
    fnCreateGraphicsPipelines(device, NULL, 1, &pipelineInfo, NULL, &graphicsPipeline);

    // Command Pool with RESET bit
    PFN_vkCreateCommandPool fnCreateCommandPool = (PFN_vkCreateCommandPool)vkGetInstanceProcAddr(instance, "vkCreateCommandPool");
    VkCommandPoolCreateInfo cpInfo = { VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO, NULL, 0x00000002 /* VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT */, 0 };
    ...
    PFN_vkResetCommandBuffer fnResetCmd = (PFN_vkResetCommandBuffer)vkGetInstanceProcAddr(instance, "vkResetCommandBuffer");
    ...
    LOGI("✔ Minimal Diagnostic Render Loop Running (600 frames)...");
    ...
    LOGI("🎉 Minimal Triangle Test Finished!");
    return 0;
}
