...
def patch_avian_regex():
    import os
    regex_dir = "classpath/java/util/regex"
    if not os.path.exists(regex_dir):
        print(f"WARNING: {regex_dir} does not exist, skipping regex patch")
        return

    # 1. MatchResult.java
    mr_path = os.path.join(regex_dir, "MatchResult.java")
    if not os.path.exists(mr_path):
        mr_code = '''/* Generated by patch-avian-ios.py */
...
'''
        with open(mr_path, "w") as f:
            f.write(mr_code)
        print("Created classpath/java/util/regex/MatchResult.java")

    # 2. PatternSyntaxException.java
    pse_path = os.path.join(regex_dir, "PatternSyntaxException.java")
    if not os.path.exists(pse_path):
        pse_code = '''/* Generated by patch-avian-ios.py */
package java.util.regex;
...
public class PatternSyntaxException extends IllegalArgumentException {
    ...
    public String getMessage() {
        return desc + (index >= 0 ? " near index " + index : "") + (pattern != null ? "\\n" + pattern : "");
    }
}
'''
        with open(pse_path, "w") as f:
            f.write(pse_code)
        print("Created classpath/java/util/regex/PatternSyntaxException.java")

    # 3. Matcher.java -> implements MatchResult
    matcher_path = os.path.join(regex_dir, "Matcher.java")
    if os.path.exists(matcher_path):
        with open(matcher_path, "r") as f:
            m_content = f.read()
        if "implements MatchResult" not in m_content:
            m_content = m_content.replace("public abstract class Matcher {", "public abstract class Matcher implements MatchResult {")
            with open(matcher_path, "w") as f:
                f.write(m_content)
            print("Patched Matcher.java to implement MatchResult")
